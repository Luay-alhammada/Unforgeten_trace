...
# Check for the existence of 'lat' and 'lon' columns in the filtered DataFrame.
# This prevents the KeyError if the filtering removes all rows with lat/lon data.
if 'lat' in df_filtered.columns and 'lon' in df_filtered.columns:
    # Convert 'lat' and 'lon' to numeric, handling any non-numeric values
    df_filtered["lat"] = pd.to_numeric(df_filtered["lat"], errors="coerce")
    df_filtered["lon"] = pd.to_numeric(df_filtered["lon"], errors="coerce")

    # Drop any rows where 'lat' or 'lon' became NaN after coercion
    ...
    # Display a message if the columns are missing from the filtered DataFrame
    with col2:
        st.error("أعمدة 'lat' أو 'lon' غير موجودة في البيانات المصفاة.")
...
with col8:
    ...
    ax.pie(
        ...
        startangle=90,
        textprops={'fontsize': 10, 'fontfamily': 'Tajawal'}
    )
    ax.legend(
        legend_labels,
        loc="center left",
...
# Left column: Investigators and sections
with col6:
    st.markdown("<h4>3 - المحققون والأقسام داخل الفرع</h4>", unsafe_allow_html=True)
    st.markdown("""
<p>في الفرع، تظهر البيانات وجود تصنيفين للمحققين:</p>
<ul style='list-style-type: disc; padding-right: 20px;'>
    <li>
        <strong>الفئة الأولى:</strong> تشمل المحققين الذين لم تُذكر أسماؤهم، بل رموز تشير إليهم (مثل: ث4، ز10، ل2). يُرجح أن هذا الإجراء يهدف إلى الحفاظ على سريتهم، خاصة في ظل دورهم في انتزاع الاعترافات عبر أساليب التعذيب.
    </li>
    <li>
        <strong>الفئة الثانية:</strong> تضم المحققين الذين ذُكرت أسماؤهم الأولى أو الكاملة. في بعض الحالات، أشارت السجلات إلى القسم أو اللجنة المسؤولة عن التحقيق بدلًا من اسم المحقق.
    </li>
</ul>
<br>
...
<ul style='list-style-type: disc; padding-right: 20px;'>
    <li>
        قسم التنظيمات والتيارات التكفيرية - قسم التحقيق الجنائي والاقتصادي - قسم التحقيق الأمني ومكافحة الإرهاب - قسم التحقيق الانضباطي والمسلكي.
    </li>
    <li>
        اللجنة الأولى - اللجنة الثانية - لجنة أولى + لجنة ثانية - لجنة الميدان - لجنة التحقيق مع المعادين.
    </li>
</ul>
""", unsafe_allow_html=True)
...
with col10:
    ...
    st.markdown(
        """
        <p>
        فيما يخص أماكن الاعتقال، أظهرت البيانات ما يقارب أربعين موقعًا للاعتقال أو الاحتجاز. يوضح الرسم البياني أعلاه أكثر 15 موقعًا ورودًا، مثل <b>المزة الجديد</b> و<b>المزة </b> (الذي يُشار إليه غالبًا بالقديم).
        <br>
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width()/2, height + 2, f'{int(height)}',
                ha='center', va='bottom', fontsize=14, fontfamily='Tajawal')

    ax.tick_params(axis='x', labelsize=16, rotation=45, labelrotation=45, labelcolor='#555', pad=10)
    ax.tick_params(axis='y', labelsize=14, labelcolor='#555')
...
with col12:
    ...
    st.markdown("""
<div dir='rtl'>
<p>تم ترتيب التهم من الأكثر تكراراً إلى الأقل، بناءً على البيانات المُقدمة وهذه التهم مشتركة للأطفال والكبار الذين اعتقلوا معهم:</p>
<ol>
<li><b>المشاركة في المظاهرات وأعمال الشغب:</b> تشمل هذه التهم المشاركة في المظاهرات، أعمال الشغب والتخريب، التحريض على التظاهر، مقاومة الدوريات، ترديد شعارات مناهضة للدولة، ورمي الحجارة على قوات الأمن. تتضمن بعض الحالات حمل لافتات مسيئة، أعلام، وكتابة عبارات مسيئة على الجدران.</li>
<li><b>العضوية في مجموعات إرهابية مسلحة:</b> تشمل هذه التهم الانضمام إلى جماعات مسلحة مختلفة، المشاركة في عمليات إرهابية ضد الجيش والأمن، إقامة حواجز على الطرقات، وحيازة أسلحة.</li>
<li><b>التعامل مع المسلحين:</b> تشمل هذه التهم تقديم الدعم المادي واللوجستي للمسلحين، نقل الأسلحة والذخيرة، توفير المأوى، وتقديم المعلومات.</li>
<li><b>التحريض:</b> يشمل التحريض على التظاهر، التحريض الطائفي، وتحريض العسكريين على الانشقاق.</li>
<li><b>الاشتباه بوضعهم الأمني:</b> تشمل هذه التهم مراقبة الحواجز والمنشآت الحكومية، حيازة مقاطع مسيئة على الهاتف، وعدم الامتثال لأوامر الدورية.</li>
<li><b>التواصل مع إرهابيين:</b> تشمل هذه التهم التواصل مع أفراد معروفين بانتمائهم لجماعات إرهابية، وتبادل المعلومات معهم.</li>
<li><b>مختلف:</b> تشمل هذه الفئة تهمًا متفرقة مثل سرقة، تزوير، انتحال صفة أمنية، تخريب ممتلكات عامة، محاولة مغادرة البلاد بطريقة غير شرعية.</li>
</ol>
</div>
""", unsafe_allow_html=True)
...
with col19:
    ...
    ax.set_title(get_display(arabic_reshaper.reshape(f"عدد السجلات شهرياً ({selected_year})")), fontsize=18, fontfamily='Tajawal')
    ax.set_xlabel(get_display(arabic_reshaper.reshape("الشهر")), fontsize=14, fontfamily='Tajawal')
    ax.set_ylabel(get_display(arabic_reshaper.reshape("العدد")), fontsize=14, fontfamily='Tajawal')
    ...
    st.markdown("""
    <p>
    يُظهر المخطط البياني أعلاه التوزيع الزمني لحالات الاعتقال الموثقة خلال هذه الفترة. ويكشف التحليل عن أنماط مهمة في وتيرة الاعتقالات، حيث وصلت إلى ذروتها في شهر <strong>أيار من عام 2013</strong>، مما يشير إلى تصعيد كبير في عمليات الاعتقال.
    </p>
    """, unsafe_allow_html=True)
